...
static volatile unsigned long testV1;
static volatile unsigned long testV2;
static volatile unsigned long testV3;
static volatile unsigned int count;
static struct lock *testlock2;

static void customLockTest(void *junk, unsigned long num) {
	int i;	
	(void)junk;
	for(i =0; i<300; i++) {
		lock_acquire(testlock);
...
static void releaseLocks(void *junk, unsigned long num) {
	(void) junk;
	int i;
	for (i = 0; i<1000; i++) {
		lock_release(testlock);
...
int lockTest1(int nargs, char **args) {
	(void)nargs;
	(void)args;
	int i, result;	
	inititems();
	...
	count = 0;
	kprintf("Starting custom lock test..\n");
	for (i=0; i<NTHREADS; i++) {
		result = thread_fork("synchtest", NULL, i, customLockTest,
				     NULL);
		if (result) {
			panic("locktest: thread_fork failed: %s\n",
			      strerror(result));
	...
	for (i=0; i<NTHREADS; i++) {
		result = thread_fork("synchtest", NULL, i, releaseLocks,
				     NULL);
		if (result) {
			panic("locktest: thread_fork failed: %s\n",
			      strerror(result));
	...
	kprintf("Lock threads forked\n");
	lock_acquire(testlock2);
	cv_wait(testcv, testlock2);
	lock_release(testlock2);
	kprintf("Lock threads finished\n");
